...
function onclick(e) {
  if (!e.isTrusted) //this Dblclick implementation skips script generated events (same as the native dblclick mixin)
    return;
...
export function dblclickable(el, connectDisconnect = true, duration = 300, distance = 5) {
  if (connectDisconnect) {
    el.shadowRoot.addEventListener('click', onclick, {preventable: EventListenerOptions.PREVENTABLE_SOFT, /*, todo trusted: true*/});
    //todo the browser doesn't make this preventable_soft, but i think it should..
    cache.set(el, {last: null, duration, distance});
  } else {
    el.shadowRoot.removeEventListener('click', onclick);
    cache.delete(el);                //do gc
  }
}
