...
function cancelDragging() {
  target.dispatchEvent(new DragEvent('drag-cancel', lastDragEvent), document);
...
function endDragging(mouseup) {
  ...
  target.dispatchEvent(new DragEvent('drag-end', mouseup), document);
  target.removeAttribute(':dragging', pseudo);
  target.style.userSelect = userSelectOG;
  lastDragEvent = userSelectOG = mousedown = target = undefined;
}
...
function tryToDrag(mousemove) {
  ...
  target.dispatchEvent(new DragEvent('drag-start', mousedown), document);
  target.dispatchEvent(lastDragEvent, document);
}

function mousemoveToDrag(mousemove) {
  // console.log(mousemove)
  mousemove.preventDefault(); //we do not stop the propagation of mousemove events here.
  ...
  target.dispatchEvent(lastDragEvent = new DragEvent('drag', mousemove), document);
}

export class DragMaybe extends HTMLElement {
  firstConnectedCallback() {
    this.shadowRoot.addEventListener('mousedown', maybeDragListener
      // , {
...
export class DragMaybeReaction extends HTMLElement {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if (newValue === null) {
      document.removeEventListener('mousemove', tryToDrag);
      document.removeEventListener('mouseup', cancelDragMaybe);
      document.removeEventListener('mousedown', cancelDragMaybe);
      document.removeEventListener('blur', cancelDragMaybe);
    } else {
      document.addEventListener('mousemove', tryToDrag);
      document.addEventListener('mouseup', cancelDragMaybe);
      document.addEventListener('mousedown', cancelDragMaybe);
      document.addEventListener('blur', cancelDragMaybe);
    }
  }
}
...
export class DraggingReaction extends HTMLElement {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if (newValue === null) {
      document.removeEventListener('mousemove', mousemoveToDrag);
      document.removeEventListener('mouseup', endDragging);
      document.removeEventListener('mousedown', cancelDragging);
      document.removeEventListener('blur', cancelDragging);
    } else {
      document.addEventListener('mousemove', mousemoveToDrag);
      document.addEventListener('mouseup', endDragging);
      document.addEventListener('mousedown', cancelDragging);
      document.addEventListener('blur', cancelDragging);
    }
  }
}