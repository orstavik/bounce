...
class EventLoop extends HTMLElement {
  ...
  runTask() {
    ...
    if (waitingEvent) {
      ...
      const firstTime = parseInt(waitingTimeouts[0].getAttribute(':time'));
      const timeToWait = firstTime - new Date().getTime();
      if (timeToWait <= 0) {
        runTimer(waitingTimeouts[0]);
        waitingTimeouts[0].removeAttribute(':on');
      } else {
...
function makeTimeoutElement(cb, ms, param) {
  ...
  return el;
    ...
    name ? 
    console.warn("Unable to add local function", name, "to", el) :
    console.warn("Unable to add an anonymous function to", el)
  }  
}

const setTimeoutOG = window.setTimeout;
window.setTimeout = function setTimeout(cb, ms, param) {
  const timeoutElement = makeTimeoutElement(cb, ms, param);
  document.querySelector("event-loop").prepend(timeoutElement);
};
