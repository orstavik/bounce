    ...
    if (waitingEvent) {
      ...
      for (const timeout of waitingTimeouts) {
       ...
       const timeToWait = firstTime - new Date().getTime();
        if ((initialTime - firstTime) > 20) { // resumed timeouts which are deprecated more than 20ms
          this.active = false;
...
function makeTimeoutElement(cb, ms, param) {
  ...
  //window[cb.name] = cb; //we can access function without defining it manually
  return el;
}

const setTimeoutOG = window.setTimeout;
window.setTimeout = function setTimeout(cb, ms, param) {
  document.querySelector("event-loop").prepend(makeTimeoutElement(cb, ms, param));
};
