...
describe('stopPropagation', function () {
  ...
  function log(e, capture, node) {
    ...
    return [root, b, target].map(s => (s + '           ').substr(0, 13)).join(' ').trim() + '\n';
  }
  ...
  it('stopImmediatePropagation', function () {
    ...
    for (let node of targets)
      node.addEventListener("my-event-123", e => res += log(e, true, node), true);
...
`;
  ...
  it('stopPropagation', function () {
    ...
    let res = "";
    for (let node of targets)
      node.addEventListener("my-event-234", e => res += log(e, false, node), false);
...
`;
  ...
  it('stopPropagation <inner-frame-slot> capture', function () {
    targets[7].addEventListener("my-event-5678", e => e.stopPropagation());

    let res = "";
    for (let node of targets) {
      node.addEventListener("my-event-5678", function (e) {
        res += log(e, true, node).trim() + '\n';
      }, true);
    }
    h1.dispatchEvent(new Event('my-event-5678', {composed: true, bubbles: true}));
    const test =
      `window        c            window
window        c            document
window        c            HTML
