...
//   let {path, contextChildren} = result;
//   path = depth + i + ':' + path.map(et => et.nodeName || 'window').join(',') + '\n';
//   contextChildren = contextChildren.map((c, i) => c ? replacer(null, c, depth + '..', i) : null).filter(a => a);
//   return path + contextChildren.flat();
// }
//
describe('bouncedPath', function () {

  it("<outer-host>.shadowRoot", function () {
    const result = bounceSequence(outerHost.shadowRoot, window);
    const bouncedPathString = toString(result).join('\n');
    const test =
...
..5:SLOT,#document-fragment`;
    expect(bouncedPathString).to.be.equal(test);
  });

  it("<h1> innermost nested element", function () {
    const result = bounceSequence(mostNestedH1, window);
    const bouncedPathString = toString(result).join('\n');
    const test = `0:OUTER-HOST,SPAN,LINK-SLOT,DIV,BODY,HTML,#document,window
