...
function onFirstNativeListener(e) {
  ...
  //rule : reroute all natively dispatched events to the shadowRoot of the element.
  //       todo will this conflict with .click() and .requestSubmit()? don't think so.
...
EventTarget.prototype.dispatchEvent = function (e) {
  ...
  const targetIterator = targetPhaseIterator(e, this); //todo update the way the stopPropagatiion and preventDefault gets their currentContextIndex
  // e.ititit = targetIterator;
  // targetLoop: for (let {target, phase} = nextTargetPhase(e); target; {target, phase} = nextTargetPhase(e)) {
  targetLoop: for (let next = targetIterator.next(); !next.done; next = targetIterator.next()) {
    ...
    for (let i = 0; i < listeners.length; i++) {
      ...
      //todo ensure that listener.target === currentTarget
      invokeListener(e, listener.target, listener.cb);
      if (listener.preventable === 2)                       //preventable: PREVENTABLE
        e.preventDefault();
      if (isStopped(e) === 2) {                             //stopImmediatePropagation()
        targetIterator.skipContext();
        // skipContext(e);
        continue targetLoop;
      }
    }
...
//   setPaths(e, this);
//   eventStack.unshift(e);
//   for (let listener; listener = nextListener(e, getListeners);) {
