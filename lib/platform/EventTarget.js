...
function invokeListener(e, target, cb) {
  try {
    ...
    //todo this is a default action for the window on error actually..
    // The problem is only that window doesn't have a shadowRoot? Maybe window should have a shadowRoot?
    // the window shadowRoot would always run last. then the window would be the host node for the main document.
    // this works really really nicely... wow.
    // then we would never need any post processing of events, that would always be default actions..
    // that means that we actually can clear the state of the event post propagation.
