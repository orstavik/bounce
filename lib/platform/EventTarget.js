...
function propagate(path, e, prevented, context) {
...
//shouldn't preventDefault apply to all subsequent contexts? no, preventDefault does NOT go up to **parent host** context.
...
function propagateContexts(context, e, preventedIn) {
  context.prevented = context.prevented || preventedIn;      //If a context has been prevented via host, then we don't want to overwrite it.
  ...
  propagate(context.path, e, preventedIn, context);
