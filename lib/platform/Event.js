...
function getPropagationRoot(event, target) {
...
export function setPaths(e, target) {
  ...
  const root = eventState.root || getPropagationRoot(e, target);
...
export function preventDefaultImpl(contexts, prevented, i) {
  prevented[i] = true;
  for (let j = i + 1; j < contexts.length; j++)
    contexts[j].parent === i && preventDefaultImpl(contexts, prevented, j);
}
...
export function checkAndPreventNativeDefaultAction(event) {
  ...
  for (let i = 0; i < contexts.length; i++) {
    if (i < contexts.length - 1 && !contexts[i + 1].slot)  //jump to last (innerMost) host context (and slot contexts)
      continue;
    let context = contexts[i];
    if (!context.host || !getNativeDefaultAction(context.host, event)) //no native default action, just skip
      continue;
    //now we are at the innerMost, nearest target, lowestWins default action
    state.prevented[i] && EventOG.preventDefault.call(event);
    return;
  }
}

// function checkAndPreventNativeDefaultActionBackup(event) {
//   const state = getEventState(event);
...
//     const defaultPreventedForContext = state.prevented[propagationContextIndex];
...
//this adds properties to the Event so that we can get the correct state information from it.
  ...
  'composedPath': {
    value: function () {
      const state = getEventState(this);
      return EventOG.composedPath.call(this) || state?.composedPath || [];
    }
  },
  'stopPropagation': {
    value: function () {
  ...
  'preventDefault': {
    value: function () {
      const state = getEventState(this);
      preventDefaultImpl(state.contexts, state.prevented, state.doc);
    }
  },
  'defaultPrevented': {
    get: function () {
  ...
  'preventDefaultOnHost': {
    value: function (element) {
      ...
      const state = getEventState(this);
      const shadowIndex = state.contexts.findIndex(({host, root}) => host === element || root === element.shadowRoot);
      if (shadowIndex === -1)
