...
function nextTarget(phase, path, targetIndex) { //todo rename to getCurrentTarget
...
function getEventState(e) {
  ...
  if (!state)
    cache.set(e, state = {/*doc: 0, phase: 0, target: 0,*/ stopImmediate: [], stop: [], prevented: []});
  state.ititit = e.ititit;
  return state;
}
...
//this adds properties to the Event so that we can get the correct state information from it.
  'eventPhase': {    //rule #x: adding phase 4 for event finished propagating.
    get: function () {
      return getEventState(this).ititit?.phase || Event.NONE;
  ...
  'currentTarget': {
    get: function () {
      ...
      return getEventState(this).ititit?.currentTarget || null;
      // return nextTarget(state.phase, activePropagationContext(state)?.path, state.target) || null;
    }
  },
  'path': {
    get: function () {
      const state = getEventState(this);
      return getEventState(this).ititit?.path || [];
  ...
  'target': {
    get: function () {
      const state = getEventState(this);
      return getEventState(this).ititit?.target || null;
      return activePropagationContext(state)?.path[0] || null;
    }
  },
  'currentDocument': {
    get: function () {
      const state = getEventState(this);
      return getEventState(this).ititit?.currentRoot || null;
  ...
  'stopPropagation': {
    value: function () {
      const state = getEventState(this);
      const stateDoc = getEventState(this).ititit?.currentContextIndex || 0;
      state.stop[stateDoc] = true;
  ...
  'stopImmediatePropagation': {
    value: function () {
      const state = getEventState(this);
      const stateDoc = getEventState(this).ititit?.currentContextIndex || 0;
      state.stopImmediate[stateDoc] = true;
...
export function isStopped(e) {
  ...
  const eventDoc = event.ititit?.currentContextIndex || 0;
  return event.stopImmediate[eventDoc] ? 2 : event.stop[eventDoc] ? 1 : 0;
  // return event.stopImmediate[event.doc] ? 2 : event.stop[event.doc] ? 1 : 0;
}

// export function skipContext(mainEvent) {
//   const event = getEventState(mainEvent);
//   event.doc++, event.phase = event.target = 0;
// }

export function nextTargetPhase(mainEvent) {
  const event = getEventState(mainEvent);
  event.target++;
  const contexts = event.contexts;
  for (; event.doc < contexts.length; event.doc++, event.phase = 1, event.target = 0) {
    for (; event.phase < 4; event.phase++, event.target = 0) {
      const target = mainEvent.currentTarget;
      if (target) return {target, phase: event.phase};
    }
  }
...
export function targetPhaseIterator(e, lowestTarget) {
  ...
  const event = setPaths(e, lowestTarget);
  const contexts = event.contexts;
  let currentTarget;
  let target;
  let path;
    ...
    next: function () {
      ...
      for (; contextI < contexts.length; contextI++, phase = 1, targetI = 0) {
        ...
        for (; phase < 4; phase++, targetI = 0) {
          currentTarget = nextTarget(phase, path, targetI);
          if(currentTarget)
