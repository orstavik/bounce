...
// no, preventDefault only goes up ***ONE*** *parent host context*.
// and, since all the event listener in the parent host context has already run, this will not apply to them.
// and, since all preceding nearest-target/lowest-wins sibling contexts before the current context has already run,
// preventDefault() will not apply to it.

// so, preventDefault will therefore in practice only apply to:
// 1. subsequent event listeners in the same context,
// 2. event listeners in descendant context of the current context, and
// 3. event listeners in subsequent sibling contexts of the current context (ie. other children contexts of the parent context
//    that are queued after the current context).
...
//preventDefault() general applies to the parent context.
//preventDefault(hostNode) applies specifically to one child propagation context.
//the whole point of preventDefault(hostNode) is to enable an upper slotting context to
//become effective instead of an inner slotting or target context.

//preventDefault() ONLY apply to parent (useR) context, not parent's parent.
//The direct parent can control which preventDefault actions it adds via the elements that it adds, but
//this should be overrideable from the parent's parent.
function preventDefault() {
  const state = eventToState.get(this);
  state ?
    preventContext(state.context.parent || state.context):
    preventedBeforeDispatch.add(this);
  // preventContext(state.context)
  // for (let child of SubsequentSiblingContexts(state.context))
  //   preventContext(child);
}

//todo it might be better to re-enable the default action on a host-node.
//todo and have this be stronger than preventDefault(). That way we could
//todo just apply a generalized preventDefault() and then
function preventDefaultOnHost(hostNode) {
  ...
  const childContext = state.context.contextChildren[hostIndex];
