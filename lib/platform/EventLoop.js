export {EventOG, getDefaultActionNative} from './Event.js';
import {setPaths, preventDefaultImpl} from './Event.js';
      ...
      for (; event.target < path.length; event.target++, event.listener = 0) {
        ...
        while (event.listener < listeners.length) {
          ...
          if (prevented && listener.preventable > 0)                       //PREVENTABLE_SOFT or PREVENTABLE
            continue;
          if(listener.preventable === 1 || listener.preventable === -1)    //PREVENTABLE_STRONG or PREVENTABLE
            preventDefaultImpl(contexts, event.prevented, event.doc);
          return listener;
        }
