...
<style>
  ...
  div[prop="id"], div[prop="num"], div[prop="name"] {
    display: none;
  }
...
function printObj2(prop, value, level = 1) {
  const props = {prop, level, value};
  // value instanceof Object && delete props.value;
  value instanceof Object && (props.el = value.id);
...
export class RubiksCube extends HTMLElement {
  ...
  slotchange() {
    ...
    const obj = JSON.parse(this.childNodes[0].textContent);
    const obj2 = {};
    for (let i = 0; i < obj.length; i++) {
      let val = obj[i];
      const {el, name} = val;
      obj2[`${i}_${el}_${name}`] = val;
    }
    this.shadowRoot.querySelector('slot').insertAdjacentHTML('afterend', printObj2("table", obj2));
  }
}
