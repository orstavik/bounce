...
window.HTMLEventElement = class HTMLEventElement extends HTMLElement {
  ...
  static #propagate(eventEl, target, listeners) {
    ...
    for (let context of eventEl.topMostContext) {
      //todo the defaultPrevented property is really tricky... It is communicating via the event/the variadic loop...
      //todo with the defaultPrevented running, we can't control the loop from outside..
      //todo this is a problem.
      //todo For the purity of the algorithm, then nothing should stop the actions.
      //todo but, for the sake of the mechanics, we would like to skip the iteration once the defaultPrevented is called.
      //todo but that is the
